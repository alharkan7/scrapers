#!/usr/bin/env python3
"""
Full article scraper for NU Online articles.
...
class NUOnlineFullArticleScraper:
    def __init__(self):
    ...
    def extract_full_article_data(self, url):
        """Extract all metadata and full text from an article URL."""
        ...
        # Extract date and time - specific NU Online format
        date_elem = soup.find('p', class_=lambda x: x and 'text-gray-400' in x and 'text-xs' in x)
        if date_elem and 'WIB' in date_elem.get_text():
            # Extract text and remove the span content (NU Online)
            for span in date_elem.find_all('span'):
        ...
        # Extract author and role - specific NU Online format
        author_container = soup.find('div', class_=lambda x: x and 'ml-4' in x and 'flex-1' in x)
        if author_container:
            author_name_elem = author_container.find('p', class_=lambda x: x and 'font-bold' in x)
            if author_name_elem:
                article_data['author'] = author_name_elem.get_text(strip=True)

            author_role_elem = author_container.find('p', class_=lambda x: x and 'text-sm' in x and 'text-gray-400' in x)
            if author_role_elem:
                article_data['author_role'] = author_role_elem.get_text(strip=True)

        # Extract category/tag - look for "Kesehatan" tag
        category_elem = soup.find('a', string=lambda text: text and 'Kesehatan' in text.strip())
        if category_elem:
            article_data['category'] = 'Kesehatan'

        # Extract main image and caption - specific NU Online format
        img_container = soup.find('div', class_=lambda x: x and 'relative' in x and 'aspect-video' in x)
        if img_container:
            ...
            caption_elem = img_container.find('p', class_=lambda x: x and 'text-xs' in x and 'text-gray-500' in x)
            if caption_elem:
        ...
        for selector in content_selectors:
            ...
            if content_elem:
                ...
                for p in paragraphs:
                    ...
                    if text and len(text) > 20:  # Filter out very short paragraphs (likely metadata)
        ...
        # If no structured content found, try to get all paragraphs from the page
            ...
            for p in all_paragraphs:
                ...
                if (text and len(text) > 50 and
                    not any(skip_word in text.lower() for skip_word in
                           ['copyright', 'privacy policy', 'disclaimer', 'tentang nu', 'jaringan media']) and
                    not text.startswith('Baca Juga') and
                    not text.startswith('Lihat Semua')):
                    full_text.append(text)
    ...
    def save_to_csv(self, articles, filename):
        ...
        fieldnames = ['title', 'url', 'date', 'time', 'author', 'author_role', 'category', 'image_url', 'image_caption', 'full_text', 'scraped_at']
    ...
    def scrape_full_articles_from_file(self, input_json_file, output_file=None, output_format='json'):
        ...
        for i, article_meta in enumerate(articles_metadata, 1):
            ...
            if full_article_data:
                ...
                print(f"✓ Successfully scraped article: {full_article_data.get('title', 'Unknown title')[:50]}...")
            else:
...
def scrape_full_articles(input_json_file, output_file=None, output_format='json'):
    """
    Scrape full article content from NU Online article URLs.
    ...
    scraper = NUOnlineFullArticleScraper()
    return scraper.scrape_full_articles_from_file(input_json_file, output_file, output_format)


def main():
    parser = argparse.ArgumentParser(description='Scrape full article content from NU Online article URLs')
